...
//                 // Calculate point size based on distance from camera if enabled
//                 float2 p = _PointSize * 0.001;
//                 if (_UseDistance != 0) {
//                     p *= v.w;  // Scale point size based on depth
//                 }

//                 // Adjust y scale for screen aspect ratio
//                 p.y *= _ScreenParams.x / _ScreenParams.y;

//                 // Create 4 points for the quad around the original vertex
//                 float4 offset = float4(-p.x, p.y, 0, 0);
//                 o.vertex = v + offset;
//                 o.uv = i[0].uv;
//                 triStream.Append(o);

//                 offset = float4(-p.x, -p.y, 0, 0);
//                 o.vertex = v + offset;
//                 triStream.Append(o);
...
//                 // Pass UV coordinates to geometry shader
//                 o.uv = v.uv;
//                 return o;
//             }
...
//                 // Apply UV mapping and sample the main texture
//                 float2 uv = tex2D(_UVMap, i.uv);
//                 if (any(uv <= 0 || uv >= 1))
//                     discard;
